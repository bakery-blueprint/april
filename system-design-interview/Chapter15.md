...
#### 📌 변경 사항 감지 및 저장 방식 비교 (클라이언트 vs 서버)

| 단계 | 클라이언트에서 수행 | 서버에서 수행 |
|------|------------------|--------------|
| **변경 사항 감지** | ✅ (파일 변경 감지 & 변경된 블록만 추출) | ❌ (전체 파일과 비교해야 함) |
| **변경된 블록만 추출** | ✅ (변경된 블록만 식별하여 준비) | ❌ (전체 파일을 분석해야 함) |
| **변경된 블록 전송** | ✅ (변경된 블록만 서버로 전송) | ❌ (클라이언트가 전체 파일을 전송해야 함) |
| **변경된 블록 저장** | ❌ (클라이언트는 저장 안 함) | ✅ (변경된 블록을 저장) |
| **변경된 블록 병합** | ❌ (클라이언트는 병합하지 않음) | ✅ (기존 블록과 새로운 블록 병합) |
| **이전 버전 유지 (Version Control)** | ❌ (이전 버전은 서버가 관리) | ✅ (기존 블록도 함께 저장하여 버전 관리 가능) |
| **네트워크 효율성** | ✅ (변경된 부분만 전송하여 트래픽 절감) | ❌ (전체 파일을 보내야 하므로 비효율적) |
| **서버 부하** | ✅ (클라이언트에서 변경 감지하여 서버 부담 감소) | ❌ (서버가 전체 파일을 비교해야 하므로 부하 증가) |
| **오프라인 지원** | ✅ (변경 사항을 저장해 두었다가 온라인 시 동기화 가능) | ❌ (항상 서버와 연결 필요) |

---

#### 📌 결론
- ✅ **Google Drive 같은 서비스는 "클라이언트에서 변경 감지 & 서버에서 저장" 방식 사용**  
- ✅ **네트워크 트래픽을 줄이고 서버 부하를 감소시키기 위한 최적의 방식**  
- ✅ **서버에서는 버전 관리와 변경된 블록을 병합하는 작업만 수행**  
